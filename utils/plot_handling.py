...
class Plotter:
    ...
    def get_size_input_number(self, key_name):
        while True:
            got = input(
                f"Enter the size for {key_name} (current value is {self.size_params[key_name]}): "
            )
            if got.strip() == "":
    ...
    def test_sizes(self):
        if self.parameters["figure_force_save"]:
            self.parameters["logger"].warn(
                "Parameters currently sets figure_force_save to True. This suggests you are running in an env without a display, but you cannot test sizes iteratively this way. This code may behave weirdly..."
            )
    ...
    def show(self, save_path=None, data_df=None, figure_force_save=None):
        ...
        if figure_force_save and save_path is None:
            log_error(
                "Figure force save is enabled, but no save path was provided",
                self.parameters,
            )
            return
